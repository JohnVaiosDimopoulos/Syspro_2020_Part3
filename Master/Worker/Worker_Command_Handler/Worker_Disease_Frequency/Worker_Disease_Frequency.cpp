#include "Worker_Disease_Frequency.h"

void Worker_Disease_Frequency::execute_command(System_Data *system_data,
  ...
  if(tokenized_command->get_num_of_nodes()==4){
    std::string result = system_data->get_disease_table()->diseaseFrequency(
        (*tokenized_command)[1],
