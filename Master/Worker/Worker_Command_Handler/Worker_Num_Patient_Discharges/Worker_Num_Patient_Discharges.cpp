#include "Worker_Num_Patient_Discharges.h"

void Worker_Num_Patient_Discharges::execute_command(System_Data *system_data,
                                                    Generic_List<std::string> *tokenized_command,
                                                    Query_Socket *socket) {

  if((*tokenized_command).get_num_of_nodes()==4){

    std::string result = system_data->get_disease_table()->numPatientDischarges(
        (*tokenized_command)[1],
